# TAREAS

Toda tarea debe producir un resultado verificable.

## Modo de ejecución
El sistema trabaja en cadena: al terminar una tarea, identifica y ejecuta automáticamente la siguiente tarea OPEN de mayor prioridad, salvo decisión creativa crítica del autor, información indispensable ausente, conflicto de CANON, riesgo legal/IP relevante o acción irreversible.

## Prioridades
- P0 — Crítica
- P1 — Importante
- P2 — Mejora

## Estado
OPEN / IN PROGRESS / DONE / BLOCKED

## PIPELINE — LEO-PÉREZ

| ID | Prioridad | Tarea | Estado | Resultado |
|---|---|---|---|---|
| TASK-001 | P0 | Lógica de la maldición | DONE | DEC-001 |
| TASK-002 | P0 | Identidad profunda de Leo | DONE | DEC-002 |
| TASK-003 | P0 | Identidad de Pérez | DONE | DEC-003 |
| TASK-004 | P0 | Motivación de Dintrideska | DONE | DEC-004 |
| TASK-005 | P0 | Escalada y clímax | DONE | DEC-005 |
| TASK-006 | P0 | Confusión Leo/Pérez | DONE | DEC-006 |
| TASK-007 | P0 | Título comercial | DONE | DEC-007 |
| TASK-008 | P0 | Consolidar decisiones | DONE | DEC-001 a DEC-008 |
| TASK-009 | P0 | Construir CANON-01 | DONE | CANON.md |
| TASK-010 | P0 | Reestructurar manuscrito | DONE | ESCALeta_v1.md |
| TASK-011 | P0 | Reescritura editorial v1 | DONE | MANUSCRITO_v1.md |
| TASK-012 | P0 | QA literario y coherencia | DONE | QA_LITERARIO_v1.md |
| TASK-013 | P1 | Target, extensión y paginación | IN PROGRESS | pendiente de especificación editorial |
| TASK-014 | P1 | World Bible + Visual Bible | OPEN | — |
| TASK-015 | P1 | Storyboard / Scene Sheets | OPEN | — |
| TASK-016 | P1 | Dirección de arte + prompts | OPEN | — |
| TASK-017 | P1 | Producción de ilustraciones | OPEN | — |
| TASK-018 | P1 | Maquetación + KDP | OPEN | — |
| TASK-019 | P0 | QA editorial final | OPEN | — |
| TASK-020 | P1 | Estrategia comercial + lanzamiento | OPEN | — |

## REGLA DE CONTINUIDAD
Cuando el usuario diga "continúa", "adelante", "sigue" o equivalente, el Director retoma la primera tarea IN PROGRESS/OPEN de mayor prioridad y continúa la cadena sin pedir confirmación para cada paso. Solo se detiene ante un bloqueo real o una decisión que corresponda al autor.
